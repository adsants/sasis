				...
				<div class="col-md-12">
					<div class="portlet light">
						<div class="caption">
							...
							<span class="caption-subject font-blue-sharp bold">Tambah Data Guru per Kelas</span>
						</div>
					</div>
						...
						<div class="portlet-body">
							...
							<form class="form-horizontal" method="post" action="<?=$this->template_view->base_url_admin();?>/<?=$this->uri->segment('2');?>/insert?id_tingkat_kelas=<?=$this->input->get('id_tingkat_kelas');?>&id_tahun_ajaran=<?=$this->input->get('id_tahun_ajaran');?>&id_kelas=<?=$this->input->get('id_kelas');?>">
								...
								<div class="form-group">
									...
									<div class="col-sm-5">
										<select class="form-control select2" required name="id_mata_pelajaran" id="id_mata_pelajaran" >
												<option value=""></option>
												<?php
								...
								<div class="form-group">
									<label class="control-label col-sm-3" >Pilih Guru :</label>
									<div class="col-sm-6">
										<select class="form-control select2" required name="id_pegawai" id="id_pegawai" >
												<option value=""></option>
												<?php
												foreach($this->dataGuru as $data){
												?>
												<option <?php if($this->session->flashdata('id_pegawai')== $data->id_pegawai) echo "selected";?> value="<?=$data->id_pegawai;?>"><?=$data->nip_pegawai;?> | <?=$data->nm_pegawai;?></option>
												<?php
												}
												?>
										</select>
